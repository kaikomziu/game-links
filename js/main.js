...
const GAMES = [
  {
    ...
    emoji: "🟨",
    tags: ["ネタ", "RNG", "ローグライト"],
    desc: "Level 0 から、行き先のレベル番号だけが書かれたボタンを選んで潜っていくBackrooms移動ゲーム。景色も音もヒントなし、たよれるのは番号と自分の記憶だけ。安全なレベルを選べば無傷、危険なレベルはSANITYが減り、回復レベルなら戻る。一度行ったレベルは危険度が手帳に記録され、死んで振り出しに戻っても知識は残る(周回でだんだん有利に)。脱出地点にたどり着けばクリア、行き止まりに入るとそのランは終了。レベル構成とルートはlevelコマンドで自由に組める。セーブは端末内。PC/スマホ対応。",
