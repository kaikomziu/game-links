...
const GAMES = [
  ...
  },
  {
    ...
    emoji: "💰",
    tags: ["クリッカー・放置"],
    desc: "中央のコインをクリックしてお金を稼ぐCookie Clicker風クリッカー。全20種の建物(レモネードスタンド〜銀河コングロマリット)で自動収入を増やし、全8種のクリック強化で1クリックの額を上げる。価格は買うたび1.15倍、まとめ買いx1/x10/x100対応、数字はK/M/B/T…表記。画面にランダムで出現する金貨をクリックすると『ラッキー(即時ボーナス)』『フィーバー(生産×7)』『連打フィーバー(クリック×777)』『大当たり(全体×2)』が発動。実績は237種(建物・クリック回数・総資産・転生・ゴールドバー・ツリー・金貨・プレイ時間など)、解除1個につき全生産+1%で無限に伸びる。今回の獲得額が$100万を超えると転生でき、額に応じてゴールドバー🥇を獲得→全19ノードのアップグレードツリー(クリック/自動収入倍率・購入価格減・開始資金・オフライン強化・🥇獲得量・転生スケール・金貨強化・実績ボーナス2倍)で永久強化。ツリーは🥇全額返却で振り直し可。なまえを設定すると累計獲得額で競う世界ランキング(上位50名+自分の推定順位)に参加できる。ニュースティッカー・獲得中バフ表示つき。離席中も収入継続、セーブは自動(localStorage)。PC/スマホ対応。",
