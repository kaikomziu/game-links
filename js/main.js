...
const GAMES = [
  {
    ...
    emoji: "🚇",
    tags: ["経営", "ストラテジー", "放置"],
    desc: "初期資金¥100で駅を買い、駅と駅を路線でつないで、走る電車で稼ぐ Mini Metro 風の鉄道経営ゲーム。駅をタップ→別の駅をタップで路線を敷設、終点をもう一度タップか『完了』で確定。2駅以上つなぐと列車が自動で走り出し、乗客(行き先を示す図形マークつき)を目的地まで運ぶたびに運賃が入る。区間が長いほど運賃は割り増しになり、運行しているだけでも少額の営業収入が入る。マップ上の点線の丸は購入できる駅で、買うほど価格が上がり、乗客の発生も増える。乗客が溢れた駅を放置すると赤いリングが一周してダイヤ崩壊(ゲームオーバー)。アップグレードは6種——運賃改定(運賃+50%)、車両大型化(定員+3)、加速性能(速度+22%)、増発(全路線の列車+1)、沿線開発(乗客+35%)、新規開業(路線スロット+1)。最大5路線まで拡張でき、路線網を広げるほど乗客と収入が伸びる。進行はブラウザに自動保存。PC/スマホ対応。",
  ...
  },
  {
    ...
    emoji: "🤔",
    tags: ["二択", "アンケート", "ネタ"],
    desc: "「一生、学校に行かなくていい。ただし宿題は毎日5倍」か「一生、学校には通う。ただし宿題ゼロ」か——。一生に関わる絶妙に悩む二択を、ひたすら選ぶだけ。全50問。選ぶたびに、世界の何%が同じ方を選んだかが割合バーで表示され、多数派か少数派かのバッジがつく。投票は共有サーバーで集計。最後に全50問の選択と多数派/少数派の数をまとめて振り返れる。回答は端末に保存され、途中からでも続けられる。正解はなし。PC/スマホ対応。",
