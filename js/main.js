...
const GAMES = [
  {
    ...
    emoji: "🟨",
    tags: ["ネタ", "RNG", "ローグライト"],
    desc: "Level 0 から、行き先のレベル番号だけが書かれたボタンを選んで潜っていくBackrooms移動ゲーム。景色も音も危険度表示もなし、たよれるのは番号と自分の記憶だけ。安全なレベルを選べば無傷、危険なレベルはSANITYが減り、回復レベルなら戻る。どの番号が危ないかはSANITYの増減から自分で覚えるしかない。到達記録には行ったことのあるレベルと回数だけが残り、死んで振り出しに戻っても消えない。脱出地点にたどり着けばクリア、行き止まりに入るとそのランは終了。レベル構成とルートはlevelコマンドで自由に組める。セーブは端末内。PC/スマホ対応。",
