...
const GAMES = [
  {
    ...
    emoji: "🟨",
    tags: ["ネタ", "RNG", "ローグライト"],
    desc: "Level 0 から、行き先のレベル番号だけが書かれたボタンを選んで潜っていくBackrooms移動ゲーム。Backrooms Wiki(日本語版)の実在レベル34件を収録し、ルートは各レベルの「出口」「入口」から採取。景色も音も危険度表示もなく、たよれるのは番号と自分の記憶だけ。危険なレベルほどSANITYが大きく減り、0になると振り出し。どの番号が危ないかはSANITYの増減から自分で覚えるしかない。到達記録には行ったレベルと回数だけが残り、死んでも消えない。Level 3999から「現実」へ帰還できればクリア。レベルとルートはlevelコマンドで自由に組み替え可能。セーブは端末内。PC/スマホ対応。",
